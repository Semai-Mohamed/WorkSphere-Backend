...
export class ProjectService {
    ...
    async addProject(dto : CreateProjectDto, userId : number,file : Express.Multer.File): Promise<Project> {
    ...
    async updateProject(projectId : number, dto : UpdateProjectDto,file?: Express.Multer.File): Promise<Project> {
        const project = await this.getProjectById(projectId)
        let photoUrl : string | undefined = undefined
        if(file){
          photoUrl = await this.cloudinaryStrategy.uploadFile(file, 'projects-photos') as string
        }
